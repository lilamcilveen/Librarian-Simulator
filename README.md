...
Running/using the software:
	- To run with input redirected, input:
		./T08 < in.txt
  - To play with the library yourself, you can simply just run it:
    ./T08
	- Make sure to widen the terminal window, so that the information can be displayed neatly.
	- Once it is running, a menu will come up. Select one of the menu options by inputting its corresponding letter and pressing ENTER.
